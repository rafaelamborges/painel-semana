...
export default function AgendaEventos({ embedded = false } = {}) {
  ...
  return (
    <div className="max-w-7xl mx-auto">
      <div className="flex items-baseline justify-between flex-wrap gap-3 mb-6">
        {!embedded && (
          <div>
            <p className="rotulo mb-2">Próximos 14 dias</p>
            <h1 className="page-title">Agenda</h1>
          </div>
        )}
        {embedded && <div />}
        {permissions.canAdd && (
          <button onClick={() => setShowForm(true)} className="btn-primario">
            Novo evento
          </button>
        )}
      </div>
      ...
      <div className="bg-white rounded-card border border-linha overflow-hidden">
          ...
          {days.map((day, i) => {
            ...
            return (
              <button
                ...
                className={`
                  min-h-[56px] sm:min-h-[84px] p-2 border-t border-r border-linha-suave text-left transition-colors relative
                  ${!isCurrentMonth ? 'opacity-30' : ''}
                  ${isSelected ? 'ring-2 ring-inset ring-bussola' : 'hover:bg-linha-suave/40'}
                `}
                style={dayGuardColor && isCurrentMonth ? { backgroundColor: dayGuardColor.lightHex } : {}}
              >
                <span className={`text-[13px] font-normal inline-flex w-[22px] h-[22px] items-center justify-center rounded-full ${isTodayDay ? 'bg-bussola text-white' : isCurrentMonth ? 'text-ink' : 'text-ink-mute'}`}>
                  {format(day, 'd')}
                </span>
                <div className="mt-1 space-y-0.5">
                  {dayEvents.slice(0, 2).map(ev => {
                    const evGuard = guardPattern ? getGuardForDate(parseISO(ev.start_at), guardPattern) : null
                    const evC = evGuard ? guardianColors[evGuard].hex : '#6B5CE7'
                    return (
                      <div key={ev.id} className="truncate text-[10px] px-1.5 py-0.5 rounded text-white font-normal"
      ...
      <div className="mt-5 card">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            <p className="rotulo">{format(selectedDay, "EEEE, dd/MM", { locale: ptBR }).toUpperCase()}</p>
            {selectedGuardColor && (
              <span className="flex items-center gap-2">
                <span className="ponto" style={{ backgroundColor: selectedGuardColor.hex }} />
                <span className="text-[13px] font-light text-ink-body">{guardianLabels[selectedGuard]}</span>
              </span>
            )}
          </div>
          {permissions.canAdd && (
            <button onClick={() => { setSelectedDay(selectedDay); setShowForm(true) }} className="btn-texto">+ Evento</button>
          )}
        </div>
        {selectedEvents.length === 0 ? (
          <p className="corpo">Nenhum evento neste dia.</p>
        ) : (
